...
Position
load_maze(const std::string &file_name)
{
...
{
    if (!is_valid_position(pos.row, pos.col))
    {
        return;
    }
    ...
    std::vector<Position> intermed_positions = {{-1, -1}, {-1, -1}, {-1, -1}, {-1, -1}}; // Direita, Baixo, Cima, Esquerda

    intermed_positions[0] = {pos.row, pos.col + 1}; // Direita
    intermed_positions[1] = {pos.row - 1, pos.col}; // Baixo
    intermed_positions[2] = {pos.row + 1, pos.col}; // Cima
    intermed_positions[3] = {pos.row, pos.col - 1}; // Esquerda

    std::thread W0(walk, intermed_positions[0]);
    std::thread W1(walk, intermed_positions[1]);
    std::thread W2(walk, intermed_positions[2]);
    std::thread W3(walk, intermed_positions[3]);
...
int main(int argc, char *argv[])
{
    ...
    std::thread W_main(walk, initial_pos);
    W_main.join();
